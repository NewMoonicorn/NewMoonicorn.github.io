...
function refreshData(){
    ...
    contractBalance(function(result){
        ...
        balanceElem.textContent = stripDecimals(rawStr, 3) + ' MCORN';
    });

    var userBalanceElem = document.getElementById('user-balance');
    userBalance(function(result){
        rawStr = numberWithCommas(Number(result * 1e9).toFixed(3));
        userBalanceElem.textContent = stripDecimals(rawStr, 3) + ' MCORN';
    });

    var userDivElem = document.getElementById('user-divs');
    getMyDivs(function(result){
        rawStr = numberWithCommas(Number(result).toFixed(3));
        userDivElem.textContent = stripDecimals(rawStr, 3) + ' BNB';
    });

    var userNextElem = document.getElementById('user-next');
    nextClaim(function(result){
        const unixTimestamp = result

const milliseconds = result * 1000 // 1575909015000

const dateObject = new Date(milliseconds)

const humanDateFormat = dateObject.toLocaleString() //2019-12-9 10:30:15
        rawStr = result;
        userNextElem.textContent = humanDateFormat;
    });
